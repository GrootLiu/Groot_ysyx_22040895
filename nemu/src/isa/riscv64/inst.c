...
void MachineModeExceptionReturn();
...
#define INSTPAT_MATCH(s, name, type, ... /* body */)                 \
	...
	INSTPAT("0000000 00000 00000 000 00000 11100 11", ecall, I, s->dnpc = isa_raise_intr(11, s->pc)); // R(17) is $a7
	INSTPAT("0011000 00010 00000 000 00000 11100 11", mret, R, s->dnpc = csrval(mepc), MachineModeExceptionReturn());
...
void MachineModeExceptionReturn()
{
	int temp = GetBit(cpu.csr[mstatus], 7);
	if (temp == 1)
	{
		SetBit(cpu.csr[mstatus], 3);
	}
	else
	{
		ClrBit(cpu.csr[mstatus], 3);
	}
	SetBit(cpu.csr[mstatus], 7);
	ClrBit(cpu.csr[mstatus], 11);
	ClrBit(cpu.csr[mstatus], 12);
}